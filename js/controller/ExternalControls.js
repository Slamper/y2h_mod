...
xinet.Controller.prototype.search = function(input) {
    var $j = jQuery.noConflict();
if($j.inArray(input, xlv.proteins.keys()) !== -1){
    protein = xlv.proteins.get(input);
    protein.xlv.clearSelection();
...
xinet.Controller.prototype.showinfo = function(name) {
    var $j = jQuery.noConflict();
    if(fileExists('./data/media/'+name.split("/")[0]+'.pdf')){
        xlv.popup.location.href="./data/media/"+name.split("/")[0]+".pdf";
    }
    else{
       xlv.popup.location.href="./data/media/wip.pdf"; 
    }
...
xinet.Controller.prototype.exportSVG = function(containerName) {
    ...
    var svgXml = xlv.svgElement.parentNode.innerHTML.replace(/<rect .*?\/rect>/i, "");//take out white background fill
    ...
    if (Blob) {
        ...
        xmlAsUrl = 'data:image/svg;filename=ctY2H_network.svg,';
        xmlAsUrl += encodeURIComponent(xml);
        var win = window.open(xmlAsUrl, 'ctY2H_network.svg');
    }
};
