...
ProteinLink.prototype.initSVG = function() {
    if (!this.intra) {
		...
		//this.highlightLine.setAttribute('marker-end','url(#end-arrow)');
...
ProteinLink.prototype.showID = function() {
    if (typeof send_match_ids === 'function') {
		...
		var filteredResLinkCount = filteredResLinks.length;
            ...
            var rl = filteredResLinks [0];
            var matches = rl.getFilteredMatches();
			var scores = "<h5>";
			   scores += "score: "+matches[0].score;
			scores += "</h5>";
			linkInfo += scores;
    ...
    if (typeof this.sc !== 'undefined') {
        ...
        if (this.fromProtein.form === 0 && this.toProtein.form === 0) {
            ...
            if (filteredResLinkCount > 0) {
                var matches = filteredResLinks[0].getFilteredMatches();
                this.tooltip = this.getFromProtein().name.split("/")[0]+"-"+ this.getToProtein().name.split("/")[0] + ', score: ' + matches[0].score + '';
    ...
    if (this.xlv.initComplete) {
        ...
        if (!this.shown) {
            ...
            if (this.intra) {
				...
				this.line.setAttribute('marker-end','url(#end-arrow)');
