...
function Protein(id, xlvController, acc, name, cat) {
    this.id = id; // id may not be accession
    this.xlv = xlvController;
    this.accession = acc;
    this.name = name;
...
Protein.prototype.initProtein = function (sequence, name, description) {
    ...
    if (this.name !== null & this.name !== "") {
        this.labelText = this.name.split("/")[0];
    }
        ...
        this.labelText = this.accession.split("/")[0];
    }
    else {
        this.labelText = this.id.split("/")[0];
    }
    ...
    this.getEventPoint = function (evt) {
        var p = self.xlv.svgElement.createSVGPoint();
//    var rect = this.container.getBoundingClientRect();
//   p.x = evt.clientX - rect.left;
//    p.y = evt.clientY - rect.top;
        var element = self.xlv.svgElement.parentNode;
        var top = 0, left = 0;
        do {
...
// transform the mouse-position into a position on the svg
    this.mouseToSVG = function (x, y) {
        var p = self.xlv.svgElement.createSVGPoint();
        p.x = x;
        p.y = y;
        var p = p.matrixTransform(self.xlv.container.getCTM().inverse());
        return p;
    };
    // events
    ...
    this.upperGroup.onmouseup = function (evt) {
    ...
    this.upperGroup.onmousemove = function (evt) {
        if (this.pressed === 1) {
            var p = self.getEventPoint(evt);// seems to be correct, see below
            var c = self.mouseToSVG(p.x, p.y);

            var dx = self.xlv.dragStart.x - c.x;
            var dy = self.xlv.dragStart.y - c.y;
            if (Math.sqrt(dx * dx + dy * dy) > (5 * self.xlv.z)) {
                this.state = 1;
...
Protein.prototype.addLink = function (link) {
    if (!this.proteinLinks.has(link.id)) {
        ...
        //scan for double links
        var links = this.proteinLinks.values();
        var c = links.length;
        for (var l = 0; l < c; l++) {
            var link = links[l];
            if (link.getFromProtein() === this && link.getToProtein !== this) {
                var links2 = link.getToProtein().proteinLinks.values();
                var c2 = links2.length;
                for (var l2 = 0; l2 < c2; l2++) {
                    var link2 = links2[l2];
                    if (link2 !== link && link2.getToProtein() === this && link2.getFromProtein() !== this) {
                        link2.paralink = true;
                        link.paralink = true;
                    }
...
Protein.prototype.setSelected = function (select) {
    ...
    if (select && this.isSelected === false) {
        ...
        if (this.xlv.showneighbours) {
            this.highlightneighbour();
            var protCount = xlv.proteins.values().length;
            var prots = xlv.proteins.values();
            for (var p = 0; p < protCount; p++) {
                var prot = prots[p];
                if (((prot.category.trim() === "l" && !xlv.lsu) || (prot.category.trim() === "s" && !xlv.ssu)) && !prot.isSelected) {
                    prot.setParked(true);
                }
                else if (!prot.parked) {
...
Protein.prototype.highlightneighbour = function () {
    ...
    for (var l = 0; l < linkCount; l++) {
        var link = links[l];
        if (link.intra === false) { // TODO: temp hack for mycoplasma
            neighbours.push(link.getOtherEnd(this));
        }
    }
...
Protein.prototype.setAllLineCoordinates = function () {
    ...
    for (var l = 0; l < c; l++) {
        ...
        if (link.getFromProtein().form === 0 && link.getToProtein().form === 0) {
            ...
            link.getToProtein().setLineCoordinates(link);
        }
        else {
        ...
        if (link.getFromProtein() === this) {
            ...
            if (this.form === 0) {
                if (link.getFromProtein().form === 0) {
                    ...
                    if (link.paralink === true) {
                        var deltaX = this.x - link.getFromProtein().x,
                            deltaY = this.y - link.getFromProtein().y,
                            dist = Math.sqrt(deltaX * deltaX + deltaY * deltaY),
                            normX = deltaX / dist,
                            normY = deltaY / dist,
                            sourcePadding = link.getFromProtein().getBlobRadius() * this.xlv.z + (2 * this.xlv.z),
                            targetPadding = this.getBlobRadius() * this.xlv.z + (2 * this.xlv.z),
                            sourceX = link.getFromProtein().x + (normY * (link.getFromProtein().getBlobRadius() * this.xlv.z / 2)),
                            sourceY = link.getFromProtein().y + (normX * (link.getFromProtein().getBlobRadius() * this.xlv.z / 2)),
                            targetX = this.x + ((this.getBlobRadius() * this.xlv.z) / 2 * normY),
                            targetY = this.y + ((this.getBlobRadius() * this.xlv.z) / 2 * normX),
                            targetX_padding = targetX - (targetPadding * normX),
                            targetY_padding = targetY - (targetPadding * normY);
                        link.line.setAttribute("x1", sourceX);
                        link.line.setAttribute("y1", sourceY);
                        link.line.setAttribute("x2", targetX_padding);
                        link.line.setAttribute("y2", targetY_padding);

                        link.fatLine.setAttribute("x1", sourceX);
                        link.fatLine.setAttribute("y1", sourceY);
                        link.fatLine.setAttribute("x2", targetX_padding);
                        link.fatLine.setAttribute("y2", targetY_padding);

                        link.highlightLine.setAttribute("x1", sourceX);
                        link.highlightLine.setAttribute("y1", sourceY);
                        link.highlightLine.setAttribute("x2", targetX_padding);
                        link.highlightLine.setAttribute("y2", targetY_padding);
                    }
                    else {
                        var deltaX = this.x - link.getFromProtein().x,
                            deltaY = this.y - link.getFromProtein().y,
                            ...
                            sourcePadding = link.getFromProtein().getBlobRadius() * this.xlv.z + (2 * this.xlv.z),
                            targetPadding = this.getBlobRadius() * this.xlv.z + (2 * this.xlv.z),
                            sourceX_padding = link.getFromProtein().x + (normX * sourcePadding),
                            sourceY_padding = link.getFromProtein().y + (normY * sourcePadding),
                            targetX_padding = this.x - (targetPadding * normX),
                            targetY_padding = this.y - (targetPadding * normY);
                        link.line.setAttribute("x1", sourceX_padding);
                        link.line.setAttribute("y1", sourceY_padding);
                        link.line.setAttribute("x2", targetX_padding);
                        link.line.setAttribute("y2", targetY_padding);

                        link.fatLine.setAttribute("x1", sourceX_padding);
                        link.fatLine.setAttribute("y1", sourceY_padding);
                        link.fatLine.setAttribute("x2", targetX_padding);
                        link.fatLine.setAttribute("y2", targetY_padding);

                        link.highlightLine.setAttribute("x1", sourceX_padding);
                        link.highlightLine.setAttribute("y1", sourceY_padding);
                        link.highlightLine.setAttribute("x2", targetX_padding);
                        link.highlightLine.setAttribute("y2", targetY_padding);
                    }
